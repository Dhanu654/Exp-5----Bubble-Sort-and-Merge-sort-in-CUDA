...
int main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        for (int j = 0; j < 2; j++) {
            int blockSize = blockSize_array[j];
            int numBlocks = (n + blockSize - 1) / blockSize;

            printf("\nArray Size:%d\nBlock Size:%d\nNum Blocks:%d\n", n, blockSize, numBlocks);

            for (int k = 0; k < n; k++) arr[k] = rand() % 1000;
            bubbleSortCPU(arr, n);
...
with open("sort_cuda.cu", "w") as f:
    f.write(cuda_code)

# Compile and run the CUDA code
!nvcc -std=c++11 -o sort_cuda sort_cuda.cu
!./sort_cuda
~~~

## OUTPUT:
![image](https://github.com/user-attachments/assets/64fb2976-87c4-495d-9c77-d9ece8feefc4)
